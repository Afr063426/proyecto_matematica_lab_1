...
>>>>>>> 7c1920529abad39b87608ead6b0cd8da647e9aee
    ...
    if (hombre) {
      anualidad <-
        (1-j*Axn(ListaDeTablasHombres[[(Edad + 1)]], x = Edad,  i=interes,  k = 12, m=(65-Edad) ))/d
        #llevar a futuro el salario
        ultimo_salario<-ultimo_salario*(1+0.05)^(65-Edad)
        
    } else{
      anualidad <-
        (1-j*Axn(ListaDeTablasMujeres[[(Edad + 1)]], x = Edad,  i=interes,  k = 12, m=(62-Edad) ))/d
        #llevar a futuro el salario
...
montohPensionMensualDetertinistico[i]<-PensionMensualDetertinistico(afil_hombres$Edad[i],T, afil_hombres$Antiguedad[i], afil_hombres$DEVENGADO[i], 0.06)
...
# montoMPensionMensualDetertinistico<-mapply(FUN=PensionMensualDetertinistico,Edad=afil_mujeres$Edad,hombre=F, antiguedad=afil_mujeres$Antiguedad,ultimo_salario=afil_mujeres$DEVENGADO, interes=0.06 ) 


for (i in 1:length( afil_mujeres$Edad) ){
montoMPensionMensualDetertinistico[i]<-PensionMensualDetertinistico(afil_mujeres$Edad[i],T, afil_mujeres$Antiguedad[i], afil_mujeres$DEVENGADO[i], 0.06)
montoMSeguroVidaDetertinistico[i]<-SeguroVidaDetertinistico (afil_mujeres$Edad[i], F) 
montoMSumaDeRetiroDetertinistico[i]<-SumaDeRetiroDetertinistico (afil_mujeres$Antiguedad[i],F ,afil_mujeres$Edad[i],afil_mujeres$DEVENGADO[i])
}
