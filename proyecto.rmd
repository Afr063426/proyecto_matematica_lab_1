...
#PensionMensualDetertinistico <-function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario) {
...
#Pension mensual deterministico segunda prueba
PensionMensualDetertinistico_afil<-function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario){
  
  v<-1/(1+tasa_descuento)
  aumento<-(1+0.03)*(1+tasa_cre_salario)
  j<-1/12#seq(1/12,116-55,1/12)
  if(hombre){
  ultimo_salario<-ultimo_salario*(v*aumento)^(max(65-Edad,0))
  ultimo_salario<-ultimo_salario*(antiguedad(antiguedad+max(65-Edad,0))[1])
  probabilidad<-pxt(ListaDeTablasHombres[[(Edad+1)]], x=Edad,t=j)
  suma<-0
  while(probabilidad>0){
    suma<-suma+v^j*ultimo_salario*probabilidad
    ultimo_salario<-ultimo_salario*(1+0.03)^(floor(j)==j)
    j<-j+1/12
    probabilidad<-pxt(ListaDeTablasHombres[[(Edad+1)]], x=Edad,t=j)
    }
  }else{
    ultimo_salario<-ultimo_salario*(v*aumento)^(max(62-Edad,0))
    ultimo_salario<-ultimo_salario*(antiguedad(antiguedad+max(62-Edad,0))[1])
    probabilidad<-pxt(ListaDeTablasMujeres[[(Edad+1)]], x=Edad,t=j)
  suma<-0
  while(probabilidad>0){
    suma<-suma+v^j*ultimo_salario*probabilidad
    ultimo_salario<-ultimo_salario*(1+0.03)^(floor(j)==j)
...
PensionMensualDetertinistico_pen<-function(Edad, hombre, ultimo_salario, tasa_descuento, tasa_cre_salario){
...
montohPensionMensualDetertinistico <-
  mapply(
    PensionMensualDetertinistico_afil,
    afil_hombres$Edad,
    TRUE,
...
pension_pensionados_h<-
mapply(
    PensionMensualDetertinistico_pen,
    pen_hombres$Edad,
    TRUE,
