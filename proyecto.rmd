...
PensionMensualDeterministico <-
  function(Edad, hombre, antiguedad1, ultimo_salario, tasa_descuento, tasa_cre_salario) {
    ...
    anualidad <-pxt(ListaDeTablasHombres[[(Edad+1)]], x=Edad,t=vectorMensual )*(v)^(vectorMensual)
    ...
    anualidad <-pxt(ListaDeTablasMujeres[[(Edad+1)]], x=Edad,t=vectorMensual )*(v)^(vectorMensual)
    #monto a pagar con ajuste inflacionario
    MontoAPagar <-MontoAPagar*(1+0.03)^(vectorAnualmensual)
    suma<-anualidad*MontoAPagar
...
# MontoM <- mapply(SeguroVidaDeterministico, pen_mujeres$Edad, T)
# sum(
...
TablamontosDeterministicos<-CalculosDeterministicos(0.06,0.02)
...
SensibilidadSalario <- data.frame(salarios,MontosSal,CambioSal)


Sensibilidadtasa <- data.frame(tasas,MontosDesc,CambioTasa)
...
ggplot(Sensibilidadtasa)+geom_line(aes(tasas,MontosDesc))+theme(panel.grid = element_blank(),panel.background = element_blank(),axis.line = element_line(colour = "black") )+scale_x_continuous(breaks = seq(0.01,0.12,0.01),labels=c(seq(0.01,0.12,0.01)), limits = c(0.01,0.13))+labs(y = "Costo",
       x = "Tasas de descuento",
       title = "Sensibilidad del costo a distintas tasas de descuento")+theme(plot.title = element_text(hjust = .5),plot.caption = element_text(hjust = 0))

ggplot(SensibilidadSalario)+geom_line(aes(salarios,MontosSal))+theme(panel.grid = element_blank(),panel.background = element_blank(),axis.line = element_line(colour = "black") )+scale_x_continuous(breaks = seq(0,0.12,0.01),labels=c(seq(0,0.12,0.01)), limits = c(0,0.13))+labs(y = "Costo",
       x = "Tasa de crecimiento de salarios",
       title = "Sensibilidad del costo a distintas tasas de crecimiento de salarios")+theme(plot.title = element_text(hjust = .5),plot.caption = element_text(hjust = 0))
