...
for (j in 1:(length(TablaDinamica_lxsHombres)-1)){
    ...
    ListaDeTablasHombres[[j]] <- c(with(TablaDinamica_lxsHombres,
                       new(
                         "actuarialtable",
                         interest = 0.08,
...
for (j in 1:(length(TablaDinamica_lxsMujeres)-1)){
    ...
    ListaDeTablasMujeres[[j]] <- c(with(TablaDinamica_lxsMujeres,
                       new(
                         "actuarialtable",
                         interest = 0.08,
...
#Se elimina las tablas de las mujeres

rm(TablaDinamica_lxsHombres, TablaDinamica_lxsMujeres)
```
...
##Consultar a la profe si el monto de la pension que aparece de cuando es
```
...
qx<-function(edad,anho,tabla,tiempo){
    ...
    prob_muerte<-tabla[entrada+tiempo,edad+tiempo]
...
j,m,simulaciones,seguro=1000000,inflacion){
    ...
    for(i in 1:nrow(tabla)){
        edad<-base_de_datos$FEC_NAC[i]
        salario<-base_de_datos$MON_PENSION #Consulta hay que ajustar la pension o no
        suma<-suma+pension(anho,edad,tabla,t_descuento,
        j,salario,m,simulaciones,seguro=1000000,inflacion)/simulaciones
    }
    return(suma)
}
...
costo_plan<-function(anho,mujeres,hombres,t_descuento,inflacion,aumento,afiliados,pensionados,simulaciones,m){
    ...
    sumar_mujeres<-costo_afiliados_seguro(anho,mujeres,afil_mujeres,t_descuento,inflacion,aumento,62,simulaciones,m)
                    +pensionados(anho,mujeres,pen_mujeres,t_descuento,1,m,simulaciones,seguro=1000000,inflacion)
    sumar_hombres<-costo_afiliados_seguro(anho,hombres,afil_hombres,t_descuento,inflacion,aumento,65,simulaciones,m)
                    +pensionados(anho,hombres,pen_hombres,t_descuento,1,m,simulaciones,seguro=1000000,inflacion)
    return(suma_hombres+suma_mujeres)
}
...
write.ta1ble(calculo_prueba,file="prueba.txt",sep="")
