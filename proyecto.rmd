...
pension<-function(base_de_datos,sex,t_descuento,m,simulaciones,inflacion,aumento_real,pensionado,jubilacion){
    edades<-base_de_datos$Edad
    v<-1/(1+t_descuento)
    aumento<-(1+inflacion)*(1+aumento_real)
    if(pensionado){
    montos<-base_de_datos$MON_PENSION*(aumento*v)^(max(jubilacion-edades,0))
    Antiguedades<-0
    }else{
      montos<-base_de_datos$DEVENGADO
      Antiguedades<-base_de_datos$Antiguedad
      montos<-montos*(aumento*v)^(max(jubilacion-edades,0))
    }
    fn2<-function(edad,monto,anti){
      ...
      suma<-0
      simulacion<-simulaciones
      monto<-monto *(pensionado+!pensionado*antiguedad(anti+max(jubilacion-edad))[1])
      vivos<-1
      while(simulacion>0){
        if(sex){
...
costo_plan<-function(t_descuento,inflacion,aumento_real,afiliados,pensionados,simulaciones,m){
    ...
    hombres_afil_lum<-0#lump_sum_st(afil_hombres,t_descuento,simulaciones,inflacion,aumento_real,65)
    hombres_afil_vida<-0#seguro(TRUE,afil_hombres,t_descuento,inflacion,aumento_real,65,simulaciones)
    hombres_afil_pen<-pension(afil_hombres,TRUE,t_descuento,m,simulaciones,inflacion,aumento_real,FALSE,65)
    mujeres_afil_lum<-0#lump_sum_st(afil_mujeres,t_descuento,simulaciones,inflacion,aumento_real,62)
    mujeres_afil_vida<-0#seguro(FALSE,afil_hombres,t_descuento,inflacion,aumento_real,65,simulaciones)
    mujeres_afil_pen<-0#pension(afil_hombres,FALSE,t_descuento,m,simulaciones,inflacion,aumento_real,FALSE,62)
    


    ##Pensionados
    hombres_pen_vida<-0#seguro(TRUE,pen_hombres,t_descuento,inflacion,aumento_real,65,simulaciones)
    hombres_pen_pen<-0#pension(pen_hombres,TRUE,t_descuento,m,simulaciones,inflacion,aumento_real,TRUE,65)
    mujeres_pen_vida<-0#seguro(FALSE,pen_mujeres,t_descuento,inflacion,aumento_real,65,simulaciones)
    mujeres_pen_pen<-0#pension(pen_mujeres,FALSE,t_descuento,m,simulaciones,inflacion,aumento_real,TRUE,62)
    #+seguro(sex=TRUE,base_de_datos=afil_hombres,t_descuento,inflacion,aumento,jubilacion=65,simulaciones)+
...
calculo_prueba<-costo_plan(t_descuento=0.06,inflacion=0.03,aumento_real=0.02,Afiliados_31_12_2020,Pensionados_31_12_2020,simulaciones=100,m=12)
...
PensionMensualDetertinistico <-
  function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario) {
    ...
    if (hombre) {
     ...
     anualidad <-pxt(ListaDeTablasHombres[[(Edad+1)]], 
     x=Edad,t=vectorMensual )*(1/(1+tasa_descuento))^(vectorMensual)
     anualidad<-sum((1+0.03)^(vectorAnualmensual)*anualidad)*
     (1/(1+tasa_descuento))^(max(65-Edad,0))
...
#llevar a futuro el salario
      ...
      anualidad <-pxt(ListaDeTablasMujeres[[(Edad+1)]],
      Edad,t=vectorMensual )*(1/(1+tasa_descuento))^(vectorMensual)
      anualidad <-sum((1+0.03)^(vectorAnualmensual)*anualidad)
        #llevar a futuro el salario
        ultimo_salario<-ultimo_salario*tasa_cre_salario^(max(62-Edad,0))*
        (1/(1+tasa_descuento))^(min(Edad-65,0))
