...
seguro<-function(anho,tabla,base_de_datos,t_descuento,inflacion,aumento,jubilacion,simulaciones,m){
    ...
    fn1<-function(edad){###Me quede aqui
    ...
    ######
        #print(edad)
         #ultimo_salario<-devengado*aumento_nominal^(jubilacion-edad)
         #antiguedad<-antiguedad(Antiguedad)
         #pension<-antiguedad[1]*ultimo_salario
         #aux<-aux+antiguedad[2]*ultimo_salario*simulacion
         #+v^(jubilacion-edad)*pension(anho,edad,tabla,t_descuento,Edad,pension,m,simulacion,seguro=1000000,inflacion,columna)
...
pension<-function(anho,edad,tabla,t_descuento,Edad,salario,m,simulaciones,seguro=1000000,inflacion,columna){
    simulacion<-simulaciones
    ...
    while(simulacion>0) {
      if(tabla){
      probabilidad<-pxt(ListaDeTablasHombres[[(Edad+1)]], x=edad,t=j)
      ...
      probabilidad<-pxt(ListaDeTablasHombres[[(Edad+1)]], x=edad,t=1/12)
      #probabilidad<-probabilidad*(1/12*(probabilidad!=1)+1/12*(probabilidad==1&meses<12)+(probabilidad==1 & meses==12))
        vivos<-sum(rbinom(n=simulacion,size=1,prob=probabilidad))
...
pension<-function(tabla,base_de_datos,t_descuento,m,simulaciones,inflacion,pensionado){
  if(pensionado){
    edades<-base_de_datos$Edad
    ...
    valores<-mapply(FUN=fn2,edad=edades,monto=montos)
    return(sum(valores))
  }
}
...
pensionados<-function(anho,tabla,base_de_datos,t_descuento,m,simulaciones,seguro=1000000,inflacion){
    ...
    #fn2<-function(edad,salario){
        ...
        #monto<-pension(anho,edad,tabla,t_descuento,edad,salario=salario,m,simulaciones,seguro=1000000,inflacion,columna)
    #E}
    ...
    #return(suma)
    #Este queda hay que revisar valores monto<-seguro(anho,tabla,base_de_datos,t_descuento,inflacion,0.02,65,simulaciones,12)
    monto<-pension(tabla,base_de_datos,t_descuento,m,simulaciones,inflacion,TRUE)
    return(monto)
}
...
costo_plan<-function(anho,mujeres,hombres,t_descuento,inflacion,aumento,afiliados,pensionados,simulaciones,m){
    ...
    sumar_mujeres<-0#pensionados(anho,FALSE,pen_mujeres,t_descuento,m,simulaciones,seguro=1000000,inflacion)#costo_afiliados_seguro(anho,FALSE,afil_mujeres,t_descuento,inflacion,aumento,62,simulaciones,m)+
    sumar_hombres<-pensionados(anho,TRUE,pen_hombres,t_descuento,m,simulaciones,seguro=1000000,inflacion)#seguro(anho,T,afil_hombres,t_descuento,inflacion,aumento,62,simulaciones,m)#
    return(sumar_mujeres+sumar_hombres)#costo_afiliados_seguro(anho,TRUE,afil_hombres,t_descuento,inflacion,aumento,65,simulaciones,m)+
}
...
PensionMensualDetertinistico <-
  function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario) {
    ...
    if (hombre) {
     #anualidad imnediata
     vectorMensual<-(seq(1/12, (115-Edad), 1/12))
     vectorAnualmensual<-sort(rep(c(0:(115-Edad-1)),12))
     anualidad <-pxt(ListaDeTablasHombres[[(Edad+1)]], x=Edad,t=vectorMensual )*(1/(1+0.06))^(vectorMensual)
     sum((1+0.03)^(vectorAnualmensual)*anualidad)
     
      # anualidadAcumulada <-1
      # anualidad<-0
      # for (k in vectorMensual) {
      #   anualidad <-anualidad+ pxt(ListaDeTablasHombres[[(Edad+1)]], x=Edad,t=k )*(1/(1+0.06))^(k)* anualidadAcumulada
      #   anualidadAcumulada <-anualidadAcumulada * (1+0.03)^(floor(k)==k)
      # 
      # }
      # anualidad <-12*axn(ListaDeTablasHombres[[(Edad + 1)]], x = Edad,  i=tasa_descuento,  k = 12, m=max(65-Edad,0), payment="immediate" )
      #
```
```{r}
#llevar a futuro el salario
      ...
      anualidad <-pxt(ListaDeTablasMujeres[[(Edad+1)]], Edad,t=vectorMensual )*(1/(1+0.06))^(vectorMensual)
      anualidad <-sum((1+0.03)^(vectorAnualmensual)*anualidad)
        #llevar a futuro el salario
...
SumaDeRetiroDetertinistico <- function(antiguedad, hombre, Edad, ultimo_salario,tasa_descuento, tasa_cre_salario) {
  ...
  Multiplo <- function(antiguedad) {
   ...
   if (hombre) {
        ...
        Monto<-Monto*(1+tasa_descuento)^(Edad-65)
      ...
      Monto<-ultimo_salario * Multiplo(antiguedad)
      Monto<-Monto*(1+tasa_descuento)^(Edad-62)
...
MontoM <- mapply(SeguroVidaDetertinistico, pen_mujeres$Edad, F)
sum(
  montohPensionMensualDetertinistico,
  montohSeguroVidaDetertinistico,
  montohSumaDeRetiroDetertinistico,
